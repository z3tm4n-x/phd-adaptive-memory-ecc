...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/stage-a-repro.XXXXXX")"
trap 'rm -rf "$WORK"' EXIT
CSV="$WORK/proton_rate_5min.csv"
OUT="$WORK/output"

git show 619cb3538e296b3619f21301a176665f4611143f:experiments/RE-GOES19-PROTON-RATE-01/proton_rate_5min.csv > "$CSV"
python3 "$TASK/test_input_gate.py" --config "$TASK/config.json" --frozen-csv "$CSV"
...
echo "PASS: input gate, original focused tests, deterministic science tables"
