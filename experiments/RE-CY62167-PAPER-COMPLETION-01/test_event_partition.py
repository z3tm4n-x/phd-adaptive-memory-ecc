...
class TestEventPartition(unittest.TestCase):
    ...
    def test_conservation_every_sweep_row(self):
        with (HERE/'mapping_sweep_all_series.csv').open(newline='',encoding='utf-8') as f:
            for r in csv.DictReader(f):
                self.assertGreaterEqual(int(r['S_direct_event_cells']),0)
                self.assertGreaterEqual(int(r['S_residual_cells']),0)
        with (HERE/'heavy_ion_cross_sections.csv').open(newline='',encoding='utf-8') as f:
            for r in csv.DictReader(f):
                self.assertEqual(int(r['S_direct_event_cells'])+int(r['S_accumulation']),int(r['S_cells_used']))
