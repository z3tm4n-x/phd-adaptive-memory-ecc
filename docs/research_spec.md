...
**SOURCE** — следует из утверждённого описания темы; **WORKING DEFINITION** — рабочая формализация; **ASSUMPTION** — допущение в объявленной области; **TBD** — открытый вопрос. Версия 1.1 фиксирует [решение пользователя после kill test](research_gates/RE-FIXED-ADAPTIVE-FEASIBILITY-01.md) и разрешённый единый эксперимент. [Версия 1.0 после консолидации](https://github.com/z3tm4n-x/phd-adaptive-memory-ecc/blob/abc39d5986aa7101fe2608700d455317ed683610/docs/research_spec.md) сохраняется в истории. Гипотеза остаётся непроверенной; согласование задания не переводит TBD в доказанные положения. Определения и решения [DEC-001…003](decisions/) и собственные области [RES-001…004](../results/README.md) имеют приоритет над этой сводкой.
...
Завершены ограниченный литературный этап, EXP-001, RES-002/003/004 и локальное инженерное продолжение. Старые запреты запуска уже завершённых заданий не действуют как новые оперативные gate. Консолидация завершена. Пользователь разрешил единый эксперимент RE-FIXED-ADAPTIVE-FEASIBILITY-01 по отдельной точной постановке; назначение подтверждается в постоянной сессии RE, передача через пользователя.
...
В пределах нового согласованного задания разрешены адресные аналитические/численные расчёты, сравнение, проверка общей conditional-write исполнительной модели и независимые малые эталоны. Большая архитектурная переработка, новые неизвестные параметры среды и практическая разработка RES-004 не входят в него. Приём постоянным RE ещё не подтверждён. Python-модель не является RTL-симуляцией; исторический OOC не является новым размещением или WCET. Общий SR и рефакторинг научного кода в консолидацию не входят.
...
## 10. Следующий эксперимент и публикационный контур
...
[Полная постановка](research_gates/RE-FIXED-ADAPTIVE-FEASIBILITY-01.md) · [Issue №12](https://github.com/z3tm4n-x/phd-adaptive-memory-ecc/issues/12). Задание согласовано к исполнению и **подготовлено к передаче** постоянному Research Engineer в отдельной сессии через пользователя. Приём исполнителем ещё не подтверждён; Orchestrator расчётный пакет не выполняет.

**Гипотеза — UNTESTED:** при временно неоднородной интенсивности ошибок существует предметно обоснованная область, где Fixed может обеспечить требуемый риск только с превышением доступного среднего ресурса, а причинное перераспределение выполняет совместные ограничения риска, среднего и пикового обслуживания и задержек после учёта собственной стоимости.
...
Предыдущие редакции v0.1…v0.8 и их переходы сохранены в [истории](research_map.md#история-оперативных-сводок). v1.1 означает согласованный текущий рабочий контракт, не завершение диссертации, универсальную модель SRAM или принятие всех открытых вопросов.
