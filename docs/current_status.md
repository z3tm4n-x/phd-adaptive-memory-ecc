...
**Открыт физический вопрос, а не прежний proof/executor MAJOR.** Численный CY bound=null, квалифицированные rate/задержка, full-device coverage, ненулевая физическая D3 lower и полный actual-CY m0/m1 не установлены. Прежние структурный D3/2t+1 и условная coverage-лемма сохраняются в своих областях. Зарегистрированный кластер, физический родитель, post-W событие и исход ECC различаются.

**[RE-CY62167-COVERAGE-THRESHOLD-01](research_gates/RE-CY62167-COVERAGE-THRESHOLD-01.md) принят постоянным RE в отдельной сессии; численная поставка не завершена.** [Передача BLOCKED_INPUT](https://github.com/z3tm4n-x/phd-adaptive-memory-ecc/issues/15#issuecomment-5678096574) сообщает локальный delivery e7a4c727967f752eb06ab15fe515fc38376bc896; он не объявляется опубликованным. I1/I2 и риск остаются null.  В неизменном срезе 24 h / 10 mm / main_loglog / central_mean / DREG / tau=1 s / epsilon_analysis=0.001 получить достаточную численную огибающую после SR-03, область допустимых верхних задержек записи и требуемую границу физического покрытия. Точечный rate объявленной модели отделяется от квалифицированной физической верхней интенсивности. Наличие нужного coverage-основания проверяется только по существующему корпусу. Это верхняя ветвь одной политики, не новый mapping search или попытка полного m0/m1.

В новом задании явно разрешено восстановить полный замороженный rate либо source-equivalent выбранный срез из прежних формул/входов, сохранив различие этих идентичностей. Нужный файл — direct_rate_5min.csv прежнего ECC-RISK-BRIDGE, не одноимённо трактуемый upstream proton_rate. [Уточнение 2026-09-15](research_gates/RE-CY62167-COVERAGE-THRESHOLD-01-INPUT-RECOVERY.md) разрешает только повторное получение исторического transport по закреплённому RADAR и adapter: исходный artifact 9843488846 истёк 2026-09-09, загрузка возвращает 410. Полный DREG-rate в доступных копиях является stub. Другие частные копии могут существовать; глобальная утрата не установлена. Полная сетка риска, Monte Carlo, новые транспортные модели/COSRAD и разработка контроллера не разрешены. Отрицательный остаток не является физическим запретом; отсутствующее coverage не восполняется вычитанием. Результат подлежит адресному SR. [Issue №15](https://github.com/z3tm4n-x/phd-adaptive-memory-ecc/issues/15) открыт; полного моста это поручение заранее не обещает.

После завершения ограниченного физического моста и адресного SR новых утверждений — формализация выбора наименее сложного из рассматриваемых классов управления, для которого получен достаточный сертификат выполнения требований; затем аппаратное подтверждение. [Условия завершения](research_backlog.md). Первая статья RES-003 идёт параллельно. Постоянные RE/SR работают в отдельных сессиях через пользователя; Orchestrator исследовательские проверки не выполнял.
