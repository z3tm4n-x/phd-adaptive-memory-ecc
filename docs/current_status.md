...
**Явно изменена локальная постановка продолжения:** вместо фиктивного global clean — история от записи каждого слова; finite read aperture, ERR-triggered запись и допуск часов требуют одного нового исполнительного upper U_exec. Frozen 24-h r и прежнее принятие сохраняются; pre-H rate upper вводится как явный модельный параметр, не измеренный факт. Порог 273 ns — прежний контрольный случай, не автоматический сертификат нового проекта. RE разрешено построить минимальный witness и условие осуществимости с проверяемыми ограничениями реализации. Повтор Stage 0 без нового результата не назначен. Статус продолжения — **PREPARED FOR TRANSFER; исполнение/поставку ещё не подтверждали**.

Сначала требуется результат этого ограниченного продолжения; существенные новые утверждения проходят отдельный SR. Условный проектный сертификат отделяется от фактического WCET/начала устройства. Решение о достаточности основания для joint coverage принимается Orchestrator после поставки/review, не следует из выбранных 240 ns. Затем — формализация выбора и аппаратное подтверждение. [Условия завершения](research_backlog.md). Первая статья RES-003 идёт параллельно. Orchestrator исследовательские проверки RE/SR не присваивал себе.
...
нового disposition по тому же SR не требуется. Само disposition Stage 0 не выдавало нового поручения; действующее продолжение теперь явно задано отдельным проектным дополнением выше.
