...
**Документальное дополнение 2026-09-18:** в [концепции и программе исследования](dissertation_concept.md)
зафиксированы согласованные паспорт, шесть задач, их связь с результатами и главами,
этапы обучения и план первого семестра. Это основное место формулировок цели и задач.
Дополнение не принимает новые научные результаты и не изменяет действующие поручения.

Научная сводка ниже обновлена Orchestrator 2026-09-16 в ходе исполнения DEC-004.
[DEC-004](decisions/DEC-004-dissertation-architecture-A.md) фиксирует **путь A как
основную архитектуру диссертации**. Объект — память с кодовой коррекцией ошибок;
центральный вклад — собственные методы управления при ограниченных сведениях
о сбоях и заданном вероятностном требовании. Поиск нового объекта завершён.

**Согласована [концепция и программа](dissertation_concept.md).** Общая логика —
...
**Исторический Stage 0 RE-CY62167-EXECUTOR-TIMING-GATE-01: аудит завершён и принят.** [Поставка 003c2346](../experiments/RE-CY62167-EXECUTOR-TIMING-GATE-01/REPORT.md), parent be6b447e1c2ee7b70e68604fd135b379a800e62f, содержит пять собственных файлов RE. [Disposition](research_gates/CY62167-EXECUTOR-TIMING-STAGE0-DISPOSITION-01.md): **BLOCKED_INPUT / NOT_ESTABLISHED; Stage 1 не начат**. В этой поставке раздельно не были установлены полная задержка, clean start и no-pending start; Delta_upper, lower, comparison и coverage slack были равны null. Недостаточность исполнителя не доказана. Сохранены точные bytes/история RE; Orchestrator сопоставил 27 source hashes, четыре manifest payload и ancestry, прочитал ключевой RTL, не запускал аппаратные или исследовательские расчёты.

**Проектное решение Orchestrator теперь [зафиксировано вместе с продолжением RE](research_gates/CY62167-REFERENCE-SUBSYSTEM-01.md):** AC701 Rev. 2.0 / XC7A200T-2FBG676C, три CY62167GE30-45ZXI параллельно, внешний SEC-DED(39,32), 2 MiB данных, однозначный FMC/word packing. Назначены фиксированные 24-tick слоты, чтения между ними, latch/ERR/conditional write и подготовка/fence. Это проект SRAM-расширения документированной платы, не существующее или квалифицированное устройство; изготовление стенда не назначено.

**Reference-продолжение выполнено и ограниченно принято.** Exact RE delivery [3dc58a5f79960d3ee294dc405848db5341583e0d](https://github.com/z3tm4n-x/phd-adaptive-memory-ecc/blob/3dc58a5f79960d3ee294dc405848db5341583e0d/experiments/RE-CY62167-EXECUTOR-TIMING-GATE-01/reference-continuation-01/REPORT.md), parent/preregistration 2d349dc2cf5ba5007c589652bf760a9a7682a057, следует от Stage 0. Отдельный [SR 4b8aa6ae8f8467cac2df0395603c327563796638](https://github.com/z3tm4n-x/phd-adaptive-memory-ecc/blob/4b8aa6ae8f8467cac2df0395603c327563796638/docs/scientific_reviews/CY62167_EXECUTOR_REFERENCE_REVIEW_01.md) дал PASS_WITH_MINOR без CRITICAL/MAJOR. [Disposition Orchestrator](research_gates/CY62167-EXECUTOR-REFERENCE-SR-DISPOSITION-01.md): **LIMITED CONDITIONAL ACCEPTANCE / REFERENCE SUBGATE CLOSED**. Обязательны исправленное B_H с scrub-пересечениями по H, конечный последний интервал и название корпуса 48-pin TSOP I. Поправки закреплены в disposition; исторические RE/SR и численные файлы неизменны.

Принят U_exec=0.000962653442214173738773… с положительным запасом 0.000037346557785826261227… при предполагаемом r_pre=max(r_j). Последовательная подготовка по собственным sigma_w, finite read aperture, ERR и clock/block exposure учтены; глобальная чистота в t0 не предполагается. Точные временные upper задаются u(16) и u(24), примерно 160.508 ns и 240.512 ns. Frozen r и pre-H upper не объявлены физическими огибающими. Прежний Delta_star≈273 ns остаётся отдельным контрольным случаем. Scrub резервирует 12.582912% nominal tick time интерфейса; это часть полной цены, не доказательство системного преимущества.

Reviewer независимо подтвердил арифметику и выполнил 16 Python-тестов; повторный RTL-прогон, STA и аппаратное испытание не заявлены. Orchestrator выполнил документальную проверку, не повторял исследования RE/SR. Physical timing/start/transfer остаются NOT_ESTABLISHED. Условное основание признано достаточным для постановки следующего joint physical-transfer вопроса; новое исполнение RE ещё не начато. Нужен проверяемый количественный контракт данных испытаний и полного слова, включая смешанные пути и оставшиеся условия исполнения. Остаток epsilon−U_exec не является оценкой вероятности нарушения. Затем — формальная методика выбора и исполнительное подтверждение; [критерии завершения](research_backlog.md). Первая статья RES-003 идёт параллельно.
...
нового disposition по тому же SR не требуется. Исторический Stage 0 и reference-продолжение имеют разные итоги; оба локальных этапа завершены в своих областях. Полный мост и Issue №15 не закрыты; новая серия timing-repair/re-review не назначается.
