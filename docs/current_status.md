...
Следующий предмет первого приоритета: выяснить, позволяют ли имеющиеся сведения о полноте регистрации, parent identity, parity/post-W исходах и абсолютной нормировке получить хотя бы одну проверяемую область решений. Восстановленный rate-файл и оценка задержки могут дать численную зарегистрированную upper, но не заменяют физическую полноту. Полная топология не обязательна; для неизвестности требуется назвать именно сведение, меняющее решение. [Issue №15](https://github.com/z3tm4n-x/phd-adaptive-memory-ecc/issues/15) открыт. Новая техническая задача, регенерация, поиск или стенд не запущены.
