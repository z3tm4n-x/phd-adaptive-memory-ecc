# DRAFT — NORMATIVE-BASELINE-01 clause-level extraction matrix
...
| Output status | Draft clause-level extraction; no permanent evidence or claim IDs assigned |
...
|---|---|---|---|
| `РД 134-0174-2009` | *Аппаратура радиоэлектронная бортовая космических аппаратов. Методы расчета показателей стойкости интегральных микросхем к воздействию заряженных частиц космического пространства по одиночным сбоям и отказам по результатам прямых испытаний на ускорителях заряженных частиц*. Approval/registration statements: approved in the 4th quarter of 2009, registered 2009-12-07 No. 19719; effective date 2010-07-01. 27 PDF pages. | Actual SHA-256: `272406fc0703177bd5fd59207a271033d878a218c4e08ee7fc651d3cd6bb0478`. Handoff expected `7de0db483fb6a912b0e8bcdbfff1b0cc46ff5216e0374e7cd4a05130584b8f37`. On 2026-08-30 the user explicitly authorized analysis of the supplied document and instructed that the mismatch is a processing defect. | **SOURCE:** Content identity is established from the title page and front matter. **INFERENCE:** Byte identity with the handoff-expected copy is not established; this draft records the actual analyzed file and does not treat the checksum mismatch as resolved bibliographic identity. |
| `РД 134-0175-2009` | *Аппаратура радиоэлектронная бортовая космических аппаратов. Методы испытаний цифровых сверхбольших интегральных микросхем на стойкость к воздействию отдельных высокоэнергетических протонов и тяжелых заряженных частиц космического пространства на ускорителях заряженных частиц*. Front matter states reissue under notice No. 752-02-2012 with Amendment 1; registered 2009-12-07 No. 19720; effective date 2010-07-01. 37 PDF pages. | Actual SHA-256: `4fc7f24edfa07c82de78c5b6d708ac96459e7887b0ea03e2509219ee5d89830e`. Handoff expected `5cb73d6ac1c42aa8050d669d1e303898e528785c43c6ed7ec1e9a64adf4792dd`. Same explicit user override applies. | **SOURCE:** Content identity and reissue statement are visible in the supplied full text. **INFERENCE:** Byte identity with the handoff-expected copy is not established; this draft records the actual analyzed file. |
...
**Integrity note — INFERENCE:** No scientific or normative statement below is generalized beyond the visible text of these three supplied copies. The first two checksum mismatches are retained as provenance limitations rather than silently normalized.
...
| G2 — checksum reconciliation | Source or archival record for the two handoff-expected RD SHA-256 values, or confirmation that the supplied re-generated PDFs are the intended controlled content copies | Byte-level identity and reproducibility of `РД 134-0174` and `РД 134-0175`; does not block content-bounded extraction under the user's override |
...
**Classification: `PARTIAL — NAMED INPUT NEEDED`.**

**Rationale — INFERENCE:** The three supplied documents are sufficient to extract a reproducible practical baseline through the scalar test → cross-section → environment-rate → occurrence-probability chain. They do not contain the named PMI, raw-diagnostic, target-mapping, ECC/controller, initial-state and mission inputs required to extend that chain to `W`, `E_cap(A;t0,T)` and `F_A(t0,T; μ_t0)`. The ambiguous STO copy status and the two explicitly overridden RD checksum mismatches remain provenance limitations. No claim of normative deficiency or product compliance is made.
