...
| PA-DOM-04 — Zebrev/Galimov | Reuse accepted `PAPER-005` and accepted RQ-002 synthesis; no repeated full-text extraction | Transcribe the controlled reduced inputs, reconstructed statistics, assumptions, uncertainty, mapping level and non-equivalence to DEC-001 `F_A` into the common matrix | Evidence acquisition is closed; the matrix row receives `CLOSED BY CANONICAL REUSE` unless one exact common-column proposition is absent from both accepted artefacts |
