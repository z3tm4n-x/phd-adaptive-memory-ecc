...
**[RE-CY62167-COVERAGE-THRESHOLD-01](RE-CY62167-COVERAGE-THRESHOLD-01.md) — следующее задание первого приоритета, подготовлено к передаче постоянному RE через пользователя.** Один неизменный срез; восстановление нужного численного входа явно разрешено; выход — огибающая conditional write, требуемая граница физического покрытия и статус её обоснования из существующих сведений. Локальный SR-03 не переоткрывается. Новый общий поиск, сетка сравнений, контроллер и стенд не назначены. Приём исполнителем не подтверждён.
